...
Knowledge.index = (req, res) => {
  Document.filter(r.row.hasFields("versionOf").not())
    .then((docs) => {
...
Knowledge.show = (req, res) => {
  ...
  Document.get(id)
    .getJoin({ uploads: true, versions: true })
    .then((doc) => {
...
Knowledge.toggleArchive = (req, res) => {
  const id = req.params.id;
  Document.get(id)
    .then((doc) => {
      // if 'archived' is undefined, treat it as false
      const current = doc.archived || false;
      doc.archived = !current;
      return doc.save();
    })
    .then((doc) => res.redirect(`/knowledge/${doc.id}`))
    .catch((err) => renderError(err, res));
};
...
Knowledge.save = (req, res) => {
  ...
  if (id) {
    ...
    new Document({ title, body, username })
      .save()
      .then((doc) => {
        return res.redirect(`/knowledge/${doc.id}`);
      })
